...
class FeedbackPage(QtGui.QWidget):
    ...
    toolBtnStyle = r'background-color: rgba(255, 255, 255, 0%)'
    ...
    def __init__(self):        
        ...
        closeButton.setGeometry(553, 31, 31, 31)
        closeButton.setAutoFillBackground(True)
        closeButton.setStyleSheet(self.toolBtnStyle)
        closeButton.setIcon(QtGui.QIcon(self.closeBtnPath))
        closeButton.setIconSize(QtCore.QSize(31, 31))
        closeButton.setCursor(QtCore.Qt.PointingHandCursor)
        closeButton.clicked.connect(lambda : self.hide())
    ...
    def _switchMainObjects(self, state=False):
        for i in self.mainFrame.children():
            i.setEnabled(state)
    
    def onSend(self, event):
        self.sendBtn.setText('Sending..')
        self._switchMainObjects()
        self.repaint()        
        
        worker = SendMailWorker(self.nameTxtBox.text(), self.emailTxtBox.text(),
                                self.msgTxtBox.toPlainText())
        QtCore.QObject.connect(worker, QtCore.SIGNAL('completed()'), self.onMailComplete,
